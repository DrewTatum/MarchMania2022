...
#
with open('./OutputData/season_stats_dic.json',
          'r') as infile:  # Requires running team_stats.season_stats_file previously
...
def main(year, sim_win_weight):
    ...
    for spot, team1 in enumerate(sorted_id):
        ...
        for team2 in sorted_id[spot + 1:]:
            ...
            elo_win_weight = 1 - sim_win_weight
            combined_win_prob = elo_win_weight * elo_win_prob + sim_win_weight * sim_win_prob
            # Adding results to dataframe
            final_df = final_df.append(
                {'ID': str(year) + '_' + str(team1) + '_' + str(team2), 'Pred': combined_win_prob}, ignore_index=True)
            bracket_df = bracket_df.append(
                {'ID1': str(team1), 'ID2': str(team2), 'Sim Pred': sim_win_prob, 'Elo Pred': elo_win_prob},
                ignore_index=True)
...
# Saving as cvs file
sim_win_weight = .3
df, bracket_df = main(2022, sim_win_weight)
